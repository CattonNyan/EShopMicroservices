Create one horizontal animation strip for Codex pet `xiaoba`, state `running-right`.
...
Identity: same pet in every frame: Preserve the exact existing 小八 identity and all approved standard animation rows; upgrade with a subtle natural 16-direction looking loop.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
