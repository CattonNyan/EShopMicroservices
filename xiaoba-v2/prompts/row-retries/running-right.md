Create Codex pet row `running-right` for `xiaoba`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the exact existing 小八 identity and all approved standard animation rows; upgrade with a subtle natural 16-direction looking loop.. Preserve silhouette, face, palette, material, proportions, markings, and props.
